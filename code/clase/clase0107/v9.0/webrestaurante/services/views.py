...
def create(request):
    if not request.user.has_perm('services.change_service'):
        raise PermissionDenied('No tienens privilegios para crear un servicio')
...
def order_request(request):
    ...
    try:
        ...
        print(f"Error en el parsing del JSON: {e}")
    # Se guarda en sesión el total y la orden de compra
