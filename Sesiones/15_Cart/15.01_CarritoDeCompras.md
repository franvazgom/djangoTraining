...
### 1. tamplate (html)
+ localStorage
+ JS: cart / addCart() / showBadgeCart() / orderView() 
+ HTML: form / input hidden / 
### 2. view order_request
+ POST: create table 
+ URL
### 3. tamplate detail_order
+ HTML: Table
### 4. view order_create / view order_thanks
+ Model: Order (name, address, total, email, date) 
+ Admin: CRUD
+ Form: OrderForm
+ View: order_create: 
    + session.get(total) 
    + POST: form.save()
    + GET: orderForm / initial 
+ View: order_thanks 
+ URL: order_create, order_thanks 
### 5. template order_create.html
+ HTML
+ Form: form.as_table / submit
### 6. template order_thanks 
+ HTML
